...
export default async function HomePage() {
  ...
  // });
  const usersCount = await keystoneContext
    .withSession(session)
    .query.User.count();
  ...
  return (
    <main className="flex flex-col items-center justify-between min-h-screen p-24">
      <div className="z-10 items-center justify-between w-full max-w-5xl mx-auto">
        <h1 className="text-4xl font-bold tracking-tight text-indigo-600 sm:text-5xl">
          Keystone CMS
        </h1>
        <p className="mt-4 text-lg text-gray-600">
          A simple CMS built with Keystone and Next.js
        </p>
        <p className="mt-4 text-lg text-gray-600">
          <span className="font-bold text-indigo-600">{usersCount}</span> users
        </p>
      </div>
