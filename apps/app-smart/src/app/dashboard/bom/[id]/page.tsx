...
import { useGetBOM } from 'hooks/boms';
import { objKeysToNumber } from 'lib/utils';

export default function Page() {
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const { data, mutate } = useGetBOM({ id: params?.id });
  ...
  async function submit(data: { [k: string]: FormDataEntryValue }) {
    try {
      ...
      await axios.post('/api/v1/boms/update', {
        ...objKeysToNumber(['quantity', 'price'], data),
        id: params?.id,
      });
      toast.success('You have successfully updated the BOM.');
      mutate();
      router.back();
    } catch (error) {
      toast.error(
  ...
  return (
    <section className="max-w-3xl mx-auto px-6 py-12 space-y-12">
      <PageTitle>BOM ID: {params?.id}</PageTitle>
      <FormWrapper onSubmit={submit}>
        <div className="space-y-12">
          <div className="border-b border-white/10 pb-12">
            <h2 className="text-base/7 font-semibold text-white">BOM</h2>
